...
func (r *ProductRepository) UpdateProduct(p *entities.Product) error {
	result := r.db.Model(&entities.Product{}).Where("id = ?", p.ID).Updates(p)
	if result.Error != nil {
		return result.Error
	}
...
func (r *ProductRepository) GetProductByIDWithFavorite(id uint, userID uint) (*entities.Product, bool, error) {
	var product entities.Product
	err := r.db.First(&product, id).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, false, fmt.Errorf("product with ID %d not found", id)
		}
		return nil, false, fmt.Errorf("error fetching product: %w", err)
	}
	...
	err = r.db.Table("user_favourites").
		...
		Count(&count).Error
	if err != nil {
		return nil, false, fmt.Errorf("error checking favorite status: %w", err)
	}

	// Retornar el producto con el estado de favorito
	return &product, count > 0, nil
...
func (r *ProductRepository) AddToFavorites(userID uint, productID uint) error {
	...
	// Agregar a favoritos
	if err := r.db.Model(product).Association("FavoritedBy").Append(&userEntities.User{ID: userID}); err != nil {
...
func (r *ProductRepository) RemoveFromFavorites(userID uint, productID uint) error {
	...
	if err := r.db.Model(product).Association("FavoritedBy").Delete(&userEntities.User{ID: userID}); err != nil {
		return err
	}
...
func (r *ProductRepository) GetProductsWithFavorites(userID uint, filters map[string]string) ([]entities.Product, []bool, error) {
	var products []entities.Product
	...
	if len(products) == 0 {
		return []entities.Product{}, []bool{}, nil
	}
	...
	// Crear slice de booleanos alineado con la lista de productos
	favFlags := make([]bool, len(products))
	for i, product := range products {
		favFlags[i] = contains(favoriteProductIDs, product.ID)
	}

	return products, favFlags, nil
}
