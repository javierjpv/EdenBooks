...
import (
	...
	"github.com/javierjpv/edenBooks/internal/modules/products/domain/ports/services"
...
func (u *ProductUseCase) CreateProduct(p dto.ProductRequest) error {
	...
	productEntity := entities.NewProduct(p.Name, p.Description, p.Price, p.CategoryID, p.UserID, p.ImageURL)
	return u.service.CreateProduct(productEntity)
}

func (u *ProductUseCase) UpdateProduct(id uint, p dto.ProductRequest) error {
	if p.Name == "" || p.Description == "" || p.ImageURL == "" {
		return ErrMissingFields
	}
	if p.Price <= 0 {
	...
	return u.service.UpdateProduct(productEntity)
}
...
func (u *ProductUseCase) GetProductByID(id uint, userID uint) (*dto.ProductResponse, error) {
	...
	product, isFavorite, err := u.service.GetProductByIDWithFavorite(id, userID)
	if err != nil {
		return nil, err
	}

	return dto.NewProductResponse(product, isFavorite), nil
}
...
func (u *ProductUseCase) GetProductsWithFavorites(userID uint, filters map[string]string) ([]dto.ProductResponse, error) {
	...
	products, favFlags, err := u.service.GetProductsWithFavorites(userID, filters)
	if err != nil {
		return nil, err
	}
	...
	for i := range products {
		responses[i] = *dto.NewProductResponse(&products[i], favFlags[i])
	}
