...
import (
	...
	"github.com/javierjpv/edenBooks/internal/modules/carriers/domain/ports/services"
)
...
func (u *CarrierUseCase) CreateCarrier(c dto.CarrierRequest) error {
	if c.Contact == "" || c.Name == "" {
		return ErrMissingFields
	}
	carrierEntity:=entities.NewCarrier(c.Name,c.Contact)
	return u.service.CreateCarrier(carrierEntity)
}

func (u *CarrierUseCase) UpdateCarrier(id uint, c dto.CarrierRequest) error {
	if id == 0 {
		return ErrInvalidID
	}
	if c.Contact == "" || c.Name == "" {
	...
	carrierEntity:=entities.NewCarrier(c.Name,c.Contact)
	carrierEntity.ID=id
	return u.service.UpdateCarrier(carrierEntity)
}
