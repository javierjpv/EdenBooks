...
import (
	...
	"github.com/javierjpv/edenBooks/internal/modules/messages/domain/ports/services"
)
...
func (u *MessageUseCase) CreateMessage(m dto.MessageRequest) (*entities.Message, error) {
	...
	mesaggeEntity := entities.NewMessage(m.Content, m.Seen, m.Status, m.ChatID, m.SenderID, m.ReceiverID)
	return u.service.CreateMessage(mesaggeEntity)
}
...
func (u *MessageUseCase) UpdateMessage(id uint, m dto.MessageRequest) error {
	...
	mesaggeEntity := entities.NewMessage(m.Content, m.Seen, m.Status, m.ChatID, m.SenderID, m.ReceiverID)
	return u.service.UpdateMessage(mesaggeEntity)
}
...
func (u *MessageUseCase) GetFilteredMessages(filters map[string]string) ([]dto.MessageResponse, error) {
	...
	if sortBy, exists := filters["sort_by"]; exists {
		validSortColumns := map[string]bool{"created_at": true, "updated_at": true, "seen": true, "content": true, "status": true, "chat_id": true, "sender_id": true, "receiver_id": true}
