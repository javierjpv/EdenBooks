...
import (
	...
	"github.com/javierjpv/edenBooks/internal/modules/notifications/domain/ports/services"
)
...
func (u *NotificationUseCase) CreateNotification(n dto.NotificationRequest) error {
	...
	notificationEntity:=entities.NewNotification(n.Content,n.Seen,n.UserID)
	return u.service.CreateNotification(notificationEntity)
}
...
func (u *NotificationUseCase) UpdateNotification(id uint, n dto.NotificationRequest) error {
	...
	return u.service.UpdateNotification(notificationEntity)
}
...
func (u *NotificationUseCase) GetFilteredNotifications(filters map[string]string) ([]dto.NotificationResponse, error) {
	...
	if sortBy, exists := filters["sort_by"]; exists {
		validSortColumns := map[string]bool{"created_at": true, "updated_at": true, "content": true, "seen": true,"user_id": true}
