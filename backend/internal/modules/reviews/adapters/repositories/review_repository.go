...
func (r *ReviewRepository) UpdateReview(rev *entities.Review) error {
	result := r.db.Model(&entities.Review{}).Where("id = ?", rev.ID).Updates(rev)
	if result.Error != nil {
		return result.Error
	}
...
func (r *ReviewRepository) DeleteReview(id uint) error {
	result := r.db.Delete(&entities.Review{}, id)
	if result.Error != nil {
		return result.Error
	}
	if result.RowsAffected == 0 {
