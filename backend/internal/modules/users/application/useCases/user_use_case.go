...
import (
	...
	"github.com/javierjpv/edenBooks/internal/modules/users/domain/ports/services"
)
...
func (u *UserUseCase) Register(user dto.UserRequest) error {
	...
	userEntity := entities.NewUser(user.Email, user.Password)
	return u.service.Register(*userEntity)
}
...
func (u *UserUseCase) UpdateUser(id uint, user dto.UserRequest) error {
	userEntity := entities.NewUser(user.Email, user.Password)
	userEntity.ID=id
	return u.service.UpdateUser(userEntity)
}
